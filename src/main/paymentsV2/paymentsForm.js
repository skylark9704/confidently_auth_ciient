...
const CheckoutForm = ({ productSelected, customer }) => {
  ...
  function createSubscription({ paymentMethodId }) {
    ...
    return (
      Api.post(
        "http://localhost:8000/api/v1/payments/subscription/create",
  ...
  if (accountInformation) {
    ...
    return (
      <div id="payment-form" className="flex justify-center">
        <Header>Enter Payment Information</Header>
        <Segment>
          <Header sub>Item :</Header>
          <span>{productSelected.name} Plan</span>

          <Header sub>Total Due :</Header>
          <span>{productSelected.price}$</span>
        </Segment>
        <Segment>
          <div className="w-full">
            <Input fluid transparent placeholder="Full Name" type="text" />
            <Divider />
